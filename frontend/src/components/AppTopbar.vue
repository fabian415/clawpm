    ...
    <div class="flex items-center gap-4">
      <div class="relative group">
        <button class="flex items-center gap-2 hover:opacity-80 transition-opacity">
          <div class="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-xs">JS</div>
          <span class="text-sm font-medium hidden sm:inline-block">Jason Su</span>
        ...
        <div class="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-xl py-1 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all z-50">
          ...
          <a @click.prevent="$emit('navigate', 'login')" href="#" class="block px-4 py-2 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30">登出系統</a>
        </div>
      </div>
    </div>
  </header>

...
> 預設模型 `large-v3` 約需 **6 GB VRAM**；VRAM 不足可透過 `model` 參數改用 `distil-large-v3`（~4 GB）或 `medium`（~5 GB）。
> CPU-only 模式速度較慢（即時率約 2–3x），建議搭配 `small` 或 `medium` 模型。
...
| `model` | string | `large-v3` | Whisper 模型（見下方模型對照表）|
...
| model | 精度 | 速度 | VRAM 需求 | 建議場景 |
|-------|------|------|-----------|---------|
| `large-v3` | ★★★★★ | ★★★ | ~6 GB | 高精度需求（**預設**）|
| `large-v3-turbo` | ★★★★★ | ★★★★ | ~6 GB | 高精度 + 速度兼顧 |
| `distil-large-v3` | ★★★★☆ | ★★★★★ | ~4 GB | 省 VRAM、快速 |
| `medium` | ★★★☆☆ | ★★★★ | ~5 GB | VRAM 不足時的備選 |
| `small` | ★★☆☆☆ | ★★★★★ | ~2 GB | 快速測試 |

若 `terms` 數量超過系統上限，`POST /transcribe` 回傳 HTTP 422，說明過多的 terms 可能造成 ASR hotwords/prompt 過長、記憶體壓力或 OOM。
...
在呼叫 `POST /transcribe` 時傳入較小的模型：
```bash
curl -X POST http://<server-ip>:8787/transcribe \
  -H "X-API-Key: your-secret-api-key-here" \
  -F "audio=@meeting.mp3" \
  -F "model=medium"        # 改用 medium 或 distil-large-v3
```
